...
public abstract class BlockFluid extends Block
{
    ...
    public int colorMultiplier(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            for (int var8 = -1; var8 <= 1; ++var8)
            {
                ...
                {
                    int var10 = 16777215;//par1IBlockAccess.getBiomeGenForCoords(par2 + var9, par4 + var8).waterColorMultiplier;
     ...
     */
    //TODO Make steam render as transparent and make it optional for water to boil
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if(this.blockMaterial == Material.water)
        {
        	...
        	{
        		if(
        				par1World.getBlockMaterial(par2, par3 - 2, par4) == Material.lava ||
        				par1World.getBlockMaterial(par2, par3 - 2, par4) == Material.fire ||
        				par1World.getBlockMaterial(par2 + 2, par3, par4) == Material.lava ||
        				par1World.getBlockMaterial(par2 - 2, par3, par4) == Material.lava ||
        				par1World.getBlockMaterial(par2, par3, par4 + 2) == Material.lava ||
        				par1World.getBlockMaterial(par2, par3, par4 - 2) == Material.lava 
        				)
        		{
        			this.setIsSteaming(true);
        		}
        		else
        	    {
        	     	this.setIsSteaming(false);
        	    }
        	}
        	else
    	    {
        ...
        if(isSteaming )
        {
             ...
             par1World.spawnParticle("steam", par2 + 0.5, par3 + 0.5, par4 + 0.5, 0, 0, 0);
        ...
        if(par5Random.nextInt(20) == 0)
        {
                ...
                par1World.spawnParticle("bubble", par2 + 0.5, par3 + 0.5, par4 + 0.5, 0, 0, 0);
            
        }
        
        if (par5Random.nextInt(150) == 0)
        {
        	par1World.playSound((double)par2, (double)par3, (double)par4, "liquid.bubble", 0.2F + par5Random.nextFloat() * 0.2F, 0.9F + par5Random.nextFloat() * 0.15F, false);
        }
...
/*
    ...
    protected void triggerLavaMixEffects(World par1World, int par2, int par3, int par4)
    {
        ...
        for(int i = 0; i < 8; i++)
        {
            ((WorldServer)par1World).spawnParticlePacket("largesmoke", (double)par2 + Math.random(), (double)par3 + 1.2D, (double)par4 + Math.random(), 0.0D, 0.0D, 0.0D);
        }
