...
{ protected int averageGroundLevel = -1;
     ...
     */
    protected void placeBlockAtCurrentPosition(World par1World, int blockId, int par3, int par4, int par5, int par6, StructureBoundingBox par7StructureBoundingBox)
    {
        int var8 = this.getXWithOffset(par4, par6);
        int var9 = this.getYWithOffset(par5);
        int var10 = this.getZWithOffset(par4, par6);
        int changedID = blockId;
        if(blockId > 0)
        if(par5 >= 0)
        if(par1World.rand.nextInt(2 + (par5 * 4)) == 0 || par1World.rand.nextInt(2 + (par5 * 4)) == 1)
        {
        	if(blockId == Block.cobblestone.blockID)
        	{
        		changedID = Block.cobblestoneMossy.blockID;
        	}
        	...
        	if(blockId == Block.stairsWoodOak.blockID)
        	{
        		changedID = Block.stairsRottingWood.blockID;
        	}
        }
        if (par7StructureBoundingBox.isVecInside(var8, var9, var10))
        {
            par1World.setBlock(var8, var9, var10, changedID, par3, 2);
        }
    }
     ...
     */
    protected void fillCurrentPositionBlocksDownwards(World par1World, int blockID, int par3, int par4, int par5, int par6, StructureBoundingBox par7StructureBoundingBox)
    {
        int var8 = this.getXWithOffset(par4, par6);
        int var9 = this.getYWithOffset(par5);
        int var10 = this.getZWithOffset(par4, par6);
        ...
        if (par7StructureBoundingBox.isVecInside(var8, var9, var10))
        {
            ...
            {
            	if(blockID == Block.cobblestone.blockID && par1World.rand.nextInt(2) == 0)
            	{
            		 par1World.setBlock(var8, var9, var10, Block.cobblestoneMossy.blockID, par3, 2);
            	}
            	else
            	{
                par1World.setBlock(var8, var9, var10, blockID, par3, 2);
            	}
                --var9;
            }
        }
